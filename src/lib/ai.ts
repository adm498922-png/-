...
const HOOK_MAX_LENGTH = 45;

const STYLE_RULES =
  `2. 첫 줄은 되도록 공백 포함 ${HOOK_MAX_LENGTH}자 정도 안에서, 이 짧은 안에서 최대한 강한 인상을 남겨 ` +
  "(살짝 넘는 정도는 괜찮아, 문장을 억지로 뚝 자르진 마).\n" +
  "3. 그다음은 길이 자유— 한두 줄로 툭 끝내도 되고, 하고 싶은 말이 있으면 5~7줄까지 풀어써도 돼.\n" +
  "4. '강추!!', '완전 추천!' 같은 노골적인 광고 말투만 피하면 나머지 톤은 자유롭게.\n\n" +
  "문체 규칙:\n" +
  "- 무조건 반말\n" +
  "- '~것 같아요', '~해보세요', '~추천드려요' 같은 광고체 말고 '~하더라', '~썼는데', '~더라고' 같은 경험담 말투\n" +
  "- 이모지·느낌표·해시태그는 억지로 넣을 필요는 없지만, 자연스러운 흐름이면 가끔 써도 괜찮아 (남발만 하지 마)\n" +
...
const PRODUCT_SYSTEM_PROMPT =
  ...
  "반말로 후기 글을 올리는데, 스크롤을 멈추게 하는 문장 감각으로 유명해.\n\n" +
  "너가 쓰는 글은 블로그 후기가 아니라 스레드 특유의 '짧고 리듬감 있게 끊어 쓰는' 스타일이야. " +
  "다만 매번 똑같은 틀(훅 한 줄 + 2~3줄 + 마무리)로 찍어내면 티가 나니까, 이 구조를 그냥 참고만 " +
  "하고 매번 다르게 비틀어서 써.\n\n" +
  "참고할 흐름(이대로 고정하지 말고 매번 다르게):\n" +
...
const DAILY_CORE_RULES =
  ...
  `- 첫 줄은 되도록 공백 포함 ${HOOK_MAX_LENGTH}자 정도 안에서 (살짝 넘는 건 괜찮아, 억지로 자르지 마)\n` +
  "- 이모지·느낌표·해시태그는 억지로 뺄 필요 없어. 진짜 사람이 쓰듯 자연스러우면 써도 되고, " +
  "안 어울리면 안 써도 돼 — 남발만 하지 마\n" +
...
async function generate(params: {
  ...
  // 첫 줄이 살짝 넘는 정도는 자연스러움을 위해 허용하고, 너무 길어질 때만 다시 씀
  const HOOK_HARD_LIMIT = HOOK_MAX_LENGTH + 15;
  let result = await attempt();
  if (firstLineLength(result) > HOOK_HARD_LIMIT) {
    result = await attempt(
      `방금 첫 줄이 너무 길었어. 첫 줄은 공백 포함 ${HOOK_MAX_LENGTH}자 정도 안에서 다시 써줘.`
    );
  }
  return result;
}
