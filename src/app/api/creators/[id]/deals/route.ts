...
export async function POST(
  ...
  const deal = await prisma.deal.create({
    data: {
      ...
      salesCommission: data.salesCommission ?? null,
      contentFee: data.contentFee ?? null,
      agencyFee: data.agencyFee ?? null,
      commissionRate: data.commissionRate ?? null,
      agencyRate: data.agencyRate ?? null,
      settleDueDate: data.settleDueDate ?? null,
      settledAt: data.settledAt ?? null,
      linkSent: data.linkSent ?? false,
      taxReported: data.taxReported ?? false,
      statementIssued: data.statementIssued ?? false,
      memo: data.memo ?? null,
    },
